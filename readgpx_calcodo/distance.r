...
initialbearing <- function(lat1, lon1, lat2, lon2) {
  y <- sin(torad(lon2) - torad(lon1)) * cos(torad(lat2))
  x <- cos(torad(lat1))*sin(torad(lat2)) - sin(torad(lat1))*cos(torad(lat2))*cos(torad(lon2) - torad(lon1))
  theta <- atan2(y, x)
  bear <- ((theta*180/pi) + 360) %% 360
  bear
}
